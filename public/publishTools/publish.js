...
async function getFiles(){
	var files = [];
	var accessFile = fs.readFileSync(`${config.path}/conf/access.js`).toString();
	files.push(accessFile);

	var modelDirs = fs.readdirSync(`${config.path}/models`);
	for(var i=0;i<modelDirs.length;i++){
		var pageFile = fs.readFileSync(`${config.path}/models/${modelDirs[i]}/${modelDirs[i]}Page.js`).toString();
		var modelFile = fs.readFileSync(`${config.path}/models/${modelDirs[i]}/${modelDirs[i]}Model.js`).toString();
		files.push(pageFile);
